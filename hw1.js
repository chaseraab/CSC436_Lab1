...
const Product = class Product {
    constructor(input) {
        ...
        this.price = Number(inputArray[1])
    ...
    static halfOff = (products) => {
        return products.map(inProduct => new Product(inProduct.name + "," + (inProduct.price / 2).toString() + "," + inProduct.availability))
    }
    ...
    static printProducts = (products) => {
        var dict = {"In Stock": "Yes", "Out of Stock": "No"};
        var temp = products.map(inProduct => "Product: " + inProduct.name + ", Cost: $" + inProduct.price.toFixed(2).toString() + ", Availability: " + dict[inProduct.availability])
        temp.map(inString => console.log(inString))
    }
 };
